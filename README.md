# Cocoa Programming Exercises
Practical implementations of exercises and challenges from the book **"Cocoa® Programming for Mac® OS X, Fourth Edition"** by Aaron Hillegass and Adam Preble.

## Overview
This repository contains my hands-on solutions while learning Objective-C and Cocoa framework for macOS development. Each exercise is a separate Xcode project, built, tested, and experimented with as part of the learning process.

## Structure
- Each directory corresponds to a specific chapter or exercise from the book.
- Projects are named according to the exercise topic.
- All code is manually typed, not copied, to reinforce understanding.

## Exercises
- [x] Random
- [x] lottery
- [x] Challenge: setDateFormat (p. 66)
- [x] SpeakLine
- [ ] Challenge: Count Characters (p. 96)
- [ ] Challenge: Make a Delegate (p. 114)
- [ ] Challenge: Make a Data Source (To-do list app, p. 114)

## Progress Checklist
- [x] Chapter 1: Cocoa: What Is It?
- [x] Chapter 2: Let’s Get Started
- [x] Chapter 3: Objective-C
- [x] Chapter 4: Memory Management
- [x] Chapter 5: Target/Action
- [x] Chapter 6: Helper Objects
- [ ] Chapter 7: Key-Value Coding; Key-Value Observing
- [ ] Chapter 8: NSArrayController
- [ ] Chapter 9: NSUndoManager
- [ ] Chapter 10: Archiving
- [ ] Chapter 11: Basic Core Data
- [ ] Chapter 12: NIB Files and NSWindowController
- [ ] Chapter 13: User Defaults
- [ ] Chapter 14: Using Notifications
- [ ] Chapter 15: Using Alert Panels
- [ ] Chapter 16: Localization
- [ ] Chapter 17: Custom Views
- [ ] Chapter 18: Images and Mouse Events
- [ ] Chapter 19: Keyboard Events
- [ ] Chapter 20: Drawing Text with Attributes
- [ ] Chapter 21: Pasteboards and Nil-Targeted Actions
- [ ] Chapter 22: Categories
- [ ] Chapter 23: Drag-and-Drop
- [ ] Chapter 24: NSTimer
- [ ] Chapter 25: Sheets
- [ ] Chapter 26: Creating NSFormatters
- [ ] Chapter 27: Printing
- [ ] Chapter 28: Web Services
- [ ] Chapter 29: Blocks
- [ ] Chapter 30: Developing for iOS
- [ ] Chapter 31: View Swapping
- [ ] Chapter 32: Core Data Relationships
- [ ] Chapter 33: Core Animations
- [ ] Chapter 34: Concurrency
- [ ] Chapter 35: Cocoa and OpenGL
- [ ] Chapter 36: NSTask
- [ ] Chapter 37: Distributing Your App
- [ ] Chapter 38: The End

## Purpose
This is a personal educational archive. The code may reflect my learning curve and iterative improvements.

## Book Reference
Hillegass, A., & Preble, A. (2012). *Cocoa Programming for Mac OS X* (4th ed.). Addison-Wesley.

## License
Educational use. Refer to the book for original materials.
